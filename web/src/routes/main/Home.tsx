...
function Home() {
  ...
  const previewGoals = useApiStore((state) =>
    state.getGoals(userId).slice(0, 3)
  );
    ...
    <Flex direction="column" m="md" gap="xl">
        ...
        <SimpleGrid cols={2} spacing="md">
          <Flex direction="column" align="start" gap="xs">
            <Text>
              ...
              <b>The gamified digital life companion.</b>
            </Text>
      ...
      <Flex direction="column" gap="xs">
        ...
        {previewHabits.length > 0 ? (
          <Habit
            habit={previewHabits[0]!}
            onClick={() => navigate(`/habits/${user?.username}`)}
          />
        ) : (
          <NoHabitsCard />
        )}
            ...
            <Flex direction="row" gap="xs">
              {previewMemories.map((memory) => (
                <Memory
                  key={memory.id}
                  memory={memory}
                  onClick={() => navigate(`/memories/${user?.username}`)}
                />
              ))}
            </Flex>
          </ScrollArea>
      ...
      <Flex direction="column" gap="xs">
        ...
        {previewGoals.length > 0 ? (
          <Goal
            goal={previewGoals[0]!}
            onClick={() => navigate(`/goals/${user?.username}`)}
          />
        ) : (
          <NoGoalsCard />
        )}
